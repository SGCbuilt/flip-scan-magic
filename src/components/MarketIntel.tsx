...
const GaugeCard = ({ value, label, cls = '' }: { value: string; label: string; cls?: string }) => (
  <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-center">
    <div className={`text-xl font-semibold ${cls || 'text-blue-900'}`}>{value}</div>
...
export default function MarketIntel({ results, marketStats }: Props) {
  if (!results.length) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-slate-500">
        ...
        <div className="text-sm text-slate-600">Run a search first</div>
  ...
  const grades = [
    ...
    { label: 'B (65-79)',  range: [65, 79],  color: 'bg-blue-800' },
  ...
  return (
    <div className="p-6">
      <div className="text-[10px] tracking-[2px] uppercase text-blue-900 mb-4 flex items-center gap-2">
        Market Summary
      ...
      {sd && (typeof sd === 'object') && (
        <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-4">
          <div className="text-[10px] tracking-[2px] uppercase text-blue-900 mb-3">Market Velocity (RentCast)</div>
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
        <div className="text-[10px] tracking-[2px] uppercase text-blue-900 mb-3">Score Distribution</div>
        {grades.map(g => {
          ...
          return (
            <div key={g.label} className="flex items-center gap-3 py-1.5 text-xs">
              <span className="text-slate-600 w-20">{g.label}</span>
