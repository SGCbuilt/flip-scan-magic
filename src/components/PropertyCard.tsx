...
export default function PropertyCard({ property: p, onClick, view = 'card' }: Props) {
  ...
  const accentColor = isHot ? 'bg-green-500' : 'bg-zinc-700 group-hover:bg-amber-500/60'

  if (view === 'row') {
    return (
      <tr onClick={onClick} className="border-b border-zinc-800/60 hover:bg-zinc-800/50 cursor-pointer transition-colors group">
        <td className="py-2.5 pl-4 pr-2">
          <div className="flex items-center gap-2">
            <div className={`w-1 h-8 rounded-full flex-shrink-0 ${isHot ? 'bg-green-500' : 'bg-zinc-700 group-hover:bg-amber-500'} transition-colors`} />
            <div>
              <div className="text-xs text-zinc-200 font-medium truncate max-w-[200px]">{p.addr}</div>
              <div className="text-[10px] text-zinc-600">{p.city}, {p.state} · {p.beds}bd/{p.baths}ba</div>
            </div>
          </div>
        </td>
        <td className="py-2.5 px-2 text-xs">{fmt$(p.price)}</td>
        <td className="py-2.5 px-2 text-xs text-amber-400">{fmt$(p.arv)}</td>
        ...
        <td className="py-2.5 px-2 text-xs text-zinc-400">{p.dom || '—'}</td>
  ...
  return (
    <div
      ...
      className={`group relative rounded-lg p-4 cursor-pointer transition-all overflow-hidden border
        ${isHot ? 'bg-green-950/20 border-green-800/40 hover:border-green-600/50' : 'bg-zinc-900 border-zinc-800 hover:border-amber-500/30 hover:bg-zinc-800/80'}`}
    >
      ...
      <div className="flex justify-between items-start mb-3 pl-2">
        ...
        <div className="text-right flex-shrink-0">
          ...
          <div className="text-[9px] tracking-widest uppercase text-zinc-600 mt-0.5">Score</div>
      ...
      <div className="grid grid-cols-6 gap-1.5 mb-3 pl-2">
        {[
          ...
          { label: 'ARV',         value: fmt$(p.arv),              cls: 'text-amber-400' },
          ...
          { label: 'DOM',         value: p.dom ? String(p.dom) : '—', cls: p.dom > 60 ? 'text-amber-400' : '' },
        ].map(m => (
          <div key={m.label}>
            <div className="text-[9px] tracking-wider uppercase text-zinc-600 mb-0.5">{m.label}</div>
            <div className={`text-[11px] font-semibold ${m.cls || 'text-zinc-200'}`}>{m.value}</div>
          </div>
        ))}
      </div>
      ...
      {p.signals.length > 0 && (
        <div className="pl-2 mb-2">
          <div className="text-[10px] text-zinc-600 mb-1">Opportunity Signals</div>
          <div className="flex flex-wrap gap-1">
            {p.signals.slice(0, 3).map((s, i) => (
              <span key={i} className="text-[10px] text-zinc-400 bg-zinc-800 px-2 py-0.5 rounded">{s}</span>
            ))}
            {p.signals.length > 3 && <span className="text-[10px] text-zinc-600">+{p.signals.length - 3} more</span>}
          </div>
        </div>
      )}
